...
def getSold(url, latitude1, latitude2, longitude1, longitude2):
    time.sleep(1)
    payload = {"latitude1": latitude1, "longitude1": longitude1,
...
def getSoldEdge(url, latitude1, latitude2, longitude1, longitude2, folder):
    getSoldtoFile(url, latitude2, LatitudeMax, LongitudeMin,
                  LongitudeMax, join(folder, 'north.json'))
    getSoldtoFile(url, LatitudeMin, latitude1, LongitudeMin,
                  LongitudeMax, join(folder, 'south.json'))
    getSoldtoFile(url, LatitudeMin, LatitudeMax, longitude2,
                  LongitudeMax, join(folder, 'east.json'))
    getSoldtoFile(url, LatitudeMin, LatitudeMax, LongitudeMin,
                  longitude1, join(folder, 'west.json'))
...
def getSoldInside(url, latitude1, latitude2, longitude1, longitude2, stepla, steplo, folder):
    ...
    for n in range(0, len(listla) - 1):
        for m in range(0, len(listlo) - 1):
            getSoldtoFile(url, listla[n], listla[
                          n + 1], listlo[m], listlo[m + 1], join(folder, '{0}_{1}.json'.format(n, m)))
...
def RunGetSold(name, url):
    ...
    getSoldInside(url, latitude1, latitude2, longitude1,
                  longitude2, StepLa, StepLo, folder)
...
'''
RunInsertSolds(dict_sold_fields, '20170502', dsn_raw)
RunInsertSolds(dict_sold_fields, '20170502', dsn_raw_prod)
'''


def RunInsertSolds(d, folderRaw, dsnRaw):
    for name, fields in d.items():
        folder = join('tmp', folderRaw, name)
        LogExp(InsertFromJsonFolder, folder, dsnRaw, dict_tables[name], fields)
